...
typedef struct SmallLex SmallLex;
SmallLex kr_new_small(int64_t len);
bool kr_small_at_end(SmallLex s);
SmallLex kr_small_advance(SmallLex s);
int64_t kr_tokenize_small(kr_str source, void* kinds, void* lines);
int64_t kr_main();
...
SmallLex kr_small_advance(SmallLex s) {
    return (SmallLex){.pos = _KR_ADD(s.pos, 1), .length = s.length};
}

int64_t kr_tokenize_small(kr_str source, void* kinds, void* lines) {
    SmallLex lex = kr_new_small(kr_strlen(source));
    __auto_type count = 0;
    while (!kr_small_at_end(lex)) {
        __auto_type c = kr_str_char_at(source, lex.pos);
        kr_vec_int_push(kinds, c);
        kr_vec_int_push(lines, lex.pos);
        lex = kr_small_advance(lex);
...
int64_t kr_main() {
    ...
    __auto_type count = kr_tokenize_small("hello", kinds, lines);
