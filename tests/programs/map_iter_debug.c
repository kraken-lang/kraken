...
int64_t kr_main() {
    ...
    kr_map_string_int_set(m, "a", 1);
    kr_map_string_int_set(m, "b", 2);
    int64_t map_len = kr_map_string_int_len(m);
    kr_printf("Map length: ");
    void* keys = (void*)(intptr_t)(kr_map_string_int_keys(m));
    int64_t keys_len = kr_vec_string_len(keys);
    kr_printf("Keys length: ");
    kr_map_string_int_free(m);
    kr_vec_string_free(keys);
    return keys_len - 2;
}
