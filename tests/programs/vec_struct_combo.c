...
Pair kr_make_pair(int64_t a, int64_t b);
Pair kr_next_pair(Pair p);
int64_t kr_fill_with_struct(void* v, int64_t n);
int64_t kr_main();
...
Pair kr_next_pair(Pair p) {
    return (Pair){.x = _KR_ADD(p.x, 1), .y = _KR_ADD(p.y, 1)};
}

int64_t kr_fill_with_struct(void* v, int64_t n) {
    Pair p = kr_make_pair(0, 100);
    __auto_type i = 0;
    while (i < n) {
        kr_vec_int_push(v, p.x);
        p = kr_next_pair(p);
        i = _KR_ADD(i, 1);
    }
    return n;
...
int64_t kr_main() {
    ...
    __auto_type count = kr_fill_with_struct(v, 5);
    kr_puts(kr_str_concat("count: ", kr_fmt_int((int64_t)(intptr_t)(count))));
    __auto_type i = 0;
    while (i < count) {
        kr_puts(kr_fmt_int((int64_t)(intptr_t)(kr_vec_int_get(v, i))));
        i = _KR_ADD(i, 1);
    }
    kr_vec_int_free(v);
    return 0;
