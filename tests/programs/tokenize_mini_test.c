...
Lexer kr_new_lexer(kr_str src);
bool kr_lex_at_end(Lexer lex);
Lexer kr_lex_advance(Lexer lex);
int64_t kr_lex_peek(Lexer lex);
int64_t kr_tokenize(kr_str source, void* kinds, void* lexemes, void* lines, void* cols);
int64_t kr_main();
...
Lexer kr_new_lexer(kr_str src) {
    return (Lexer){.source = src, .pos = 0, .length = kr_strlen(src), .line = 1, .column = 1};
}
...
Lexer kr_lex_advance(Lexer lex) {
    return (Lexer){.source = lex.source, .pos = _KR_ADD(lex.pos, 1), .length = lex.length, .line = lex.line, .column = _KR_ADD(lex.column, 1)};
}
...
int64_t kr_tokenize(kr_str source, void* kinds, void* lexemes, void* lines, void* cols) {
    ...
    __auto_type count = 0;
    while (!kr_lex_at_end(lex)) {
        __auto_type c = kr_lex_peek(lex);
        kr_vec_int_push(kinds, c);
        kr_vec_string_push(lexemes, "x");
        kr_vec_int_push(lines, lex.line);
        kr_vec_int_push(cols, lex.column);
        lex = kr_lex_advance(lex);
...
int64_t kr_main() {
    void* kinds = (void*)(intptr_t)(kr_vec_int_new());
    void* lexemes = (void*)(intptr_t)(kr_vec_string_new());
    void* lines = (void*)(intptr_t)(kr_vec_int_new());
    void* cols = (void*)(intptr_t)(kr_vec_int_new());
    __auto_type count = kr_tokenize("hello", kinds, lexemes, lines, cols);
    kr_puts(kr_str_concat("count: ", kr_fmt_int((int64_t)(intptr_t)(count))));
    __auto_type i = 0;
    while (i < count) {
        kr_puts(kr_fmt_int((int64_t)(intptr_t)(kr_vec_int_get(kinds, i))));
        i = _KR_ADD(i, 1);
    }
    kr_vec_int_free(kinds);
    kr_vec_string_free(lexemes);
    kr_vec_int_free(lines);
    kr_vec_int_free(cols);
    return 0;
}
