...
int64_t kr_main() {
    kr_str s = (kr_str)(intptr_t)("hello world");
    int64_t len = kr_str_len(s);
    if (_KR_NEQ(len, 11)) {
        return 1;
    }
    kr_str hello = (kr_str)(intptr_t)(kr_str_slice(s, 0, 5));
    int64_t hello_len = kr_str_len(hello);
    if (_KR_NEQ(hello_len, 5)) {
        return 2;
    }
    kr_str a = (kr_str)(intptr_t)("foo");
    kr_str b = (kr_str)(intptr_t)("bar");
    kr_str ab = (kr_str)(intptr_t)(kr_str_concat(a, b));
    int64_t ab_len = kr_str_len(ab);
    if (_KR_NEQ(ab_len, 6)) {
        return 3;
    }
    int64_t h_code = kr_str_char_at(s, 0);
    if (_KR_NEQ(h_code, 104)) {
        return 4;
    }
    int64_t w_code = kr_str_char_at(s, 6);
    if (_KR_NEQ(w_code, 119)) {
        return 5;
    }
    return 0;
}
