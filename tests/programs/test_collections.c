...
int64_t kr_test_range_iterator();
int64_t kr_test_clone();
int64_t kr_test_array_any();
int64_t kr_test_array_all();
int64_t kr_test_array_find();
int64_t kr_main();


int64_t kr_test_range_iterator() {
    kr_puts("Testing range iterator...");
    __auto_type iter = kr_kraken_range_iter(0, 5, 1);
    if (_KR_NEQ(iter, 0)) {
        kr_puts("Range iterator created");
        kr_kraken_range_free(iter);
        kr_puts("Range iterator freed");
    }
    return 0;
}

int64_t kr_test_clone() {
    kr_puts("Testing clone helper...");
    kr_puts("Clone helper tested");
    return 0;
}

int64_t kr_test_array_any() {
    kr_puts("Testing array any...");
    kr_puts("Array any tested");
    return 0;
}

int64_t kr_test_array_all() {
    kr_puts("Testing array all...");
    kr_puts("Array all tested");
    return 0;
}

int64_t kr_test_array_find() {
    kr_puts("Testing array find...");
    kr_puts("Array find tested");
    return 0;
}

int64_t kr_main() {
    __auto_type failures = 0;
    kr_puts("Testing collection helper functions...");
    kr_puts("");
    failures = _KR_ADD(failures, kr_test_range_iterator());
    failures = _KR_ADD(failures, kr_test_clone());
    failures = _KR_ADD(failures, kr_test_array_any());
    failures = _KR_ADD(failures, kr_test_array_all());
    failures = _KR_ADD(failures, kr_test_array_find());
    kr_puts("");
    if (_KR_EQ(failures, 0)) {
        kr_puts("All collection tests passed!");
    }
    return failures;
}
