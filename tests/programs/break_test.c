...
int64_t kr_find_first_even(int64_t max);
int64_t kr_skip_odds(int64_t max);
int64_t kr_main();


int64_t kr_find_first_even(int64_t max) {
    for (int64_t i = 1; i <= max; i = _KR_ADD(i, 1)) {
        if (_KR_EQ(i % 2, 0)) {
            return i;
        }
    }
...
int64_t kr_main() {
    __auto_type first = kr_find_first_even(10);
    __auto_type evens = kr_skip_odds(10);
    return _KR_ADD(first, evens);
}
