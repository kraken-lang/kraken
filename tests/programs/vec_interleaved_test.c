...
Pos kr_new_pos(int64_t n);
bool kr_pos_done(Pos p);
Pos kr_pos_next(Pos p);
int64_t kr_scan(kr_str source, void* int_data, void* lexemes);
int64_t kr_main();
...
Pos kr_pos_next(Pos p) {
    return (Pos){.idx = _KR_ADD(p.idx, 1), .len = p.len};
}

int64_t kr_scan(kr_str source, void* int_data, void* lexemes) {
    Pos p = kr_new_pos(kr_strlen(source));
    __auto_type count = 0;
    while (!kr_pos_done(p)) {
        __auto_type c = kr_str_char_at(source, p.idx);
        kr_vec_int_push(int_data, c);
        kr_vec_int_push(int_data, 1);
        kr_vec_int_push(int_data, p.idx);
...
int64_t kr_main() {
    ...
    __auto_type count = kr_scan("hello", int_data, lexemes);
    kr_puts(kr_str_concat("count: ", kr_fmt_int((int64_t)(intptr_t)(count))));
    __auto_type i = 0;
    while (i < count) {
        __auto_type kind = kr_vec_int_get(int_data, i * 3);
        __auto_type line = kr_vec_int_get(int_data, _KR_ADD(i * 3, 1));
        __auto_type col = kr_vec_int_get(int_data, _KR_ADD(i * 3, 2));
        kr_puts(kr_str_concat(kr_str_concat("char=", kr_fmt_int((int64_t)(intptr_t)(kind))), kr_str_concat(" line=", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(line)), kr_str_concat(" col=", kr_fmt_int((int64_t)(intptr_t)(col)))))));
        i = _KR_ADD(i, 1);
    }
