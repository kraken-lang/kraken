...
int64_t kr_main() {
    uint8_t* f = (uint8_t*)(intptr_t)(kr_fopen("kraken_ffi_tmp_roundtrip.bin", "wb+"));
    uint8_t* data = (uint8_t*)(intptr_t)("abcd");
    __auto_type w = kr_fwrite(data, 1, 4, f);
    if (_KR_NEQ(w, 4)) {
        return 10;
    }
    kr_rewind(f);
    uint8_t* buf = (uint8_t*)(intptr_t)(kr_malloc(4));
    __auto_type r = kr_fread(buf, 1, 4, f);
    if (_KR_NEQ(r, 4)) {
        return 11;
    }
    if (_KR_NEQ(kr_memcmp(buf, data, 4), 0)) {
        return 12;
    }
    return 0;
}
