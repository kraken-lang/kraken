...
int64_t kr_main() {
    ...
    if (_KR_NEQ(kr_vec_int_len(v), 3)) {
        return 1;
    }
    if (_KR_NEQ(kr_vec_int_get(v, 0), 10)) {
        return 2;
    }
    if (_KR_NEQ(kr_vec_int_get(v, 1), 20)) {
        return 3;
    }
    if (_KR_NEQ(kr_vec_int_get(v, 2), 30)) {
        return 4;
    }
    __auto_type popped = kr_vec_int_pop(v);
    if (_KR_NEQ(popped, 30)) {
        return 5;
    }
    if (_KR_NEQ(kr_vec_int_len(v), 2)) {
        return 6;
    }
    kr_vec_int_free(v);
    return 0;
