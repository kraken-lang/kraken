...
int64_t kr_main() {
    kr_test_section("Option and Result Tests");
    kr_test_option_some();
    kr_test_option_none();
    kr_test_result_ok();
    kr_test_result_err();
    kr_puts("All Option and Result tests passed!");
    return 0;
}
...
int64_t kr_test_option_none() {
    Option none_val = Option_None;
    if (none_val == Option_Some) {
        kr_test_fail("Should not match Some");
    }
    else if (none_val == Option_None) {
        kr_test_pass("Option::None pattern match");
    }
    return 0;
}
...
int64_t kr_test_result_err() {
    Result err_val = Result_Err;
    if (err_val == Result_Ok) {
