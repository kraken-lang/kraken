...
typedef struct CCompatible CCompatible;
typedef struct PackedStruct PackedStruct;
typedef struct AlignedStruct AlignedStruct;
int64_t kr_main();
...
int64_t kr_main() {
    CCompatible c = (CCompatible){.x = 10, .y = 20};
    PackedStruct p = (PackedStruct){.a = 1, .b = 2};
    AlignedStruct a = (AlignedStruct){.data = 42};
    return _KR_ADD(_KR_ADD(c.x, p.a), a.data);
}
