...
State kr_new_state(void* d, void* o, int64_t n);
bool kr_state_done(State s);
State kr_state_next(State s);
State kr_process(State s);
int64_t kr_main();
...
State kr_state_next(State s) {
    return (State){.pos = _KR_ADD(s.pos, 1), .len = s.len, .data = s.data, .out = s.out};
}

State kr_process(State s) {
    State current = s;
    while (!kr_state_done(current)) {
        __auto_type val = kr_vec_int_get(current.data, current.pos);
        kr_vec_string_push(current.out, kr_fmt_int((int64_t)(intptr_t)(val)));
        current = kr_state_next(current);
    }
    return current;
}

int64_t kr_main() {
    void* d = (void*)(intptr_t)(kr_vec_int_new());
    void* o = (void*)(intptr_t)(kr_vec_string_new());
    kr_vec_int_push(d, 10);
    kr_vec_int_push(d, 20);
    kr_vec_int_push(d, 30);
    State s = kr_new_state(d, o, 3);
    s = kr_process(s);
    __auto_type i = 0;
    __auto_type len = kr_vec_string_len(o);
    while (i < len) {
        kr_puts(kr_vec_string_get(o, i));
        i = _KR_ADD(i, 1);
    }
    kr_vec_int_free(d);
