...
int64_t kr_main() {
    __auto_type path1 = "kraken_ffi_tmp_ops.bin";
    __auto_type path2 = "kraken_ffi_tmp_ops_renamed.bin";
    uint8_t* f = (uint8_t*)(intptr_t)(kr_fopen(path1, "wb+"));
    if (_KR_EQ(kr_fputc(65, f), -1)) {
        return 10;
    }
    if (_KR_EQ(kr_fflush(f), -1)) {
        return 11;
    }
    if (_KR_EQ(kr_fseek(f, 0, 0), -1)) {
        return 12;
    }
    kr_fgetc(f);
    if (_KR_EQ(kr_fseek(f, 0, 2), -1)) {
        return 14;
    }
    __auto_type pos = kr_ftell(f);
    if (pos < 0) {
        return 15;
    }
    kr_feof(f);
    kr_ferror(f);
    if (_KR_EQ(kr_fclose(f), -1)) {
        return 18;
    }
    if (_KR_EQ(kr_rename(path1, path2), -1)) {
        return 19;
    }
    if (_KR_EQ(kr_remove(path2), -1)) {
        return 20;
    }
    return 0;
}
