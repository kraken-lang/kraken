...
int64_t kr_main() {
    if (!(_KR_EQ(_KR_ADD(1, 1), 2))) { fprintf(stderr, "static assertion failed: %s\n", "Basic arithmetic failed"); exit(1); }
    if (!(10 > 5)) { fprintf(stderr, "static assertion failed: %s\n", "Comparison failed"); exit(1); }
    return 0;
}
