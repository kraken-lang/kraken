...
void kr_main() {
    kr_puts("=== Channel Tests ===");
    __auto_type ch1 = kr_channel_create();
    kr_channel_try_send(ch1, 42);
    __auto_type v1 = kr_channel_try_recv(ch1);
    if (_KR_EQ(v1, 42)) {
        kr_puts("try_send/try_recv: PASS");
    }
    else {
        kr_puts("try_send/try_recv: FAIL");
    }
    kr_channel_close(ch1);
    __auto_type ch2 = kr_channel_create();
    kr_channel_send(ch2, 100);
    __auto_type v2 = kr_channel_try_recv(ch2);
    if (_KR_EQ(v2, 100)) {
        kr_puts("send/try_recv: PASS");
    }
    else {
        kr_puts("send/try_recv: FAIL");
    }
    kr_channel_close(ch2);
    __auto_type ch3 = kr_channel_create();
    kr_channel_try_send(ch3, 200);
    __auto_type v3 = kr_channel_recv(ch3);
    if (_KR_EQ(v3, 200)) {
        kr_puts("try_send/recv: PASS");
    }
    else {
        kr_puts("try_send/recv: FAIL");
    }
    kr_channel_close(ch3);
    __auto_type ch4 = kr_channel_create();
    kr_channel_send(ch4, 1);
    kr_channel_send(ch4, 2);
    kr_channel_send(ch4, 3);
    __auto_type a = kr_channel_recv(ch4);
    __auto_type b = kr_channel_recv(ch4);
    __auto_type c = kr_channel_recv(ch4);
    if (_KR_EQ(a, 1)) {
        if (_KR_EQ(b, 2)) {
            if (_KR_EQ(c, 3)) {
                kr_puts("FIFO ordering: PASS");
            }
            else {
                kr_puts("FIFO ordering: FAIL");
            }
        }
        else {
            kr_puts("FIFO ordering: FAIL");
    ...
    kr_channel_close(ch4);
    kr_puts("All channel tests complete!");
}
