...
int64_t kr_main() {
    uint8_t* token = (uint8_t*)(intptr_t)(kr_cancel_token_new());
    int64_t is_cancelled = kr_cancel_token_is_cancelled(token);
    kr_cancel_token_cancel(token);
    int64_t after_cancel = kr_cancel_token_is_cancelled(token);
    return after_cancel - _KR_ADD(1, is_cancelled);
}
