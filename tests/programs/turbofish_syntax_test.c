...
int64_t kr_test_turbofish_multiple_params() {
    ...
    KrTuple2 _kr_tup_134 = p;
    __auto_type x = _kr_tup_134.f0;
    __auto_type y = _kr_tup_134.f1;
    if (_KR_NEQ(x, 42)) {
        return 1;
    }
    if (!y) {
...
int64_t kr_main() {
    ...
    failures = _KR_ADD(failures, kr_test_turbofish_function_call());
    failures = _KR_ADD(failures, kr_test_turbofish_multiple_params());
    failures = _KR_ADD(failures, kr_test_regular_syntax_still_works());
    if (_KR_EQ(failures, 0)) {
        kr_puts("All turbofish syntax tests passed!");
    }
    return failures;
}
