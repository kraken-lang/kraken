...
- **Self-Hosted Compiler: Self-Hosting Achieved** (`krakenc/`)
...
- **Self-Hosted Parser: Semantic Registry Infrastructure** (`krakenc/src/parser.kr`) — added `build_sema_registry` pre-pass populating trait methods, impl mappings, struct fields, and function signatures into a `MapStringString` registry before forward declarations
- **Runtime: Hash Map Auto-Resize** (`krakenc/src/platform.kr`) — `MapStringInt` and `MapStringString` auto-resize at 70% load factor (initial capacity 64→256); prevents infinite-loop hangs on large registries
- **Self-Hosted Parser: Real Vtable-Based Dyn Dispatch** (`krakenc/src/parser.kr`, `krakenc/src/platform.kr`) — `KrDyn` fat pointer (`{ void* data; void* vtable; }`) replaces `void*` for dyn types; `emit_vtables` pass generates per-trait vtable structs, per-impl wrappers, and static instances; dyn vars construct fat pointers; method calls dispatch through `((Trait_VT*)obj.vtable)->method(obj.data)` — full runtime polymorphism for `dyn Trait` parameters
- **Self-Hosted Parser: Dyn Token Kind Consistency** (`krakenc/src/parser.kr`) — all `== "dyn"` lexeme checks replaced with `TK_KW_DYN()` token kind checks; backward return-type skip handles keyword return types, enabling dyn param resolution in all functions
- **Self-Hosted Parser: Nested Struct Literal Codegen** (`krakenc/src/parser.kr`) — struct literal field values that are themselves struct literals now emit proper C compound literals instead of placeholder `0`
- **Self-Hosted Parser: Untyped Local Var Method Dispatch** (`krakenc/src/parser.kr`) — method calls on untyped local variables (`let x = Foo { ... }; x.method()`) now resolve via `lookup_dyn_concrete_type` fallback
- **Self-Hosted Parser: Dyn Vtable Dispatch Args** (`krakenc/src/parser.kr`) — dyn trait method calls now translate additional arguments after `self` instead of skipping them
- **Self-Hosted Parser: Function Pointer Call Codegen** (`krakenc/src/parser.kr`) — calls through fn-typed variables and parameters now emit real C function pointer casts `((int64_t(*)(int64_t,...))name)(args)` with argument translation instead of placeholder `0`; removed hardcoded fn-var name list
- **Self-Hosted Parser: is_local_fn_var Precision** (`krakenc/src/parser.kr`) — fixed backward `TK_KW_FN` scan to skip fn-type annotations; removed overly broad `let name = identifier(...)` false-positive pattern
- **Self-Hosted Parser: Closure Capture Codegen** (`krakenc/src/parser.kr`) — closures referencing outer scope variables now work via static capture variables + `#define`/`#undef` renaming; capture assignments emitted at closure literal call site via comma expression; capture list stored in sema
- **Self-Hosted Parser: String Slice Syntax** (`krakenc/src/parser.kr`) — `name[start:end]` now emits `kr_str_slice(name, start, end)` instead of plain array indexing; pre-scans bracket contents for `:` to distinguish slicing from indexing
- **Self-Hosted Compiler: CC Flag `-Wno-int-conversion`** (`krakenc/src/main.kr`) — added to C compiler invocation to support type-erased generic functions passing strings/pointers as `int64_t`
- **Runtime: Null-Safe String Comparison** (`krakenc/src/platform.kr`) — `_kr_cmp_eq` and `_kr_cmp_neq` now check for null pointers before calling `strcmp`, preventing segfaults on `str != 0` comparisons
- **Self-Hosted Parser: skip_fn_type_sig Array Return Type** (`krakenc/src/parser.kr`) — fixed `skip_fn_type_sig` to handle array return types (`fn(T) -> [U]`); previously left `U]` unconsumed, garbling subsequent parameter names
- **Self-Hosted Parser: Closure Param/Return Type Handling** (`krakenc/src/parser.kr`) — `emit_closure_statics` now handles array types (`[T]` → `int64_t*`) and fn types (`fn(T) -> U` → `void*`) in closure annotations
- **Self-Hosted Parser: Impl Prototype Type Handling** (`krakenc/src/parser.kr`) — `emit_impl_prototypes` now calls `skip_generic_params` and `skip_fn_type_sig` on non-self params and return types; maps `[T]` to `int64_t*`
- **Self-Hosted Parser: Impl Fn Body Type Handling** (`krakenc/src/parser.kr`) — `translate_impl_fn` now matches prototype handling with `skip_generic_params`, `skip_fn_type_sig`, and `[T]` → `int64_t*`
- **Self-Hosted Parser: For-In Array Iteration** (`krakenc/src/parser.kr`) — `for item in arr { ... }` now emits proper C array iteration using `kr_vec_int_len`/`kr_vec_int_get`; detects range vs array by `..` lookahead
- **Self-Hosted Parser: Array Method Dispatch** (`krakenc/src/parser.kr`) — method calls on `[T]`-typed variables (`result.push(x)`) now emit `kr_vec_int_push(result, x)`; fixed scope scanning to traverse all enclosing scopes; fixed return-type skip for `-> [(T, U)]`; fixed param array type detection
- **Self-Hosted Parser: Fn-Type-Annotation Skip** (`krakenc/src/parser.kr`) — backward `TK_KW_FN` scan in `lookup_local_var_type_name` now skips fn-type annotations (`fn(T) -> U`), only breaking on actual function declarations
- **Self-Hosted Parser: Implicit Return** (`krakenc/src/parser.kr`) — last expression statement in non-void closure and function bodies now emits `return expr;` instead of bare `expr;`; sema-based position tracking eliminates "expression result unused" and "non-void function does not return a value" C warnings
- **Self-Hosted Parser: Tuple Type Annotation Skip** (`krakenc/src/parser.kr`) — `translate_var_decl` now skips tuple type annotations `(T, U, ...)` by matching parentheses; previously tokens leaked as bare `0;` statements
- **Self-Hosted Parser: Implicit Return `}` Boundary** (`krakenc/src/parser.kr`) — scanning now treats `}` at depth 0 as statement boundary; fixes `match { ... } expr;` patterns
- **Self-Hosted Parser: `TK_KW_UNSAFE` Fix** (`krakenc/src/parser.kr`) — `unsafe` block handler uses `TK_KW_UNSAFE()` token kind instead of broken `TK_IDENTIFIER()` string comparison
- **Self-Hosted Parser: String Literal `\!` Escape** (`krakenc/src/parser.kr`) — replaces `\!` with `!` in string literals before C emission; all 226 programs now compile with **zero C warnings**
- **Self-Hosted Parser: `str_replace` Buffer Overwrite Fix** (`krakenc/src/platform.kr`, `krakenc/src/parser.c`) — `kr_str_replace` had a critical bug: `strcpy(w,s);break;` followed by `*w=0;` overwrote copied data, returning empty strings for all non-matching inputs. This caused the self-hosted compiler to emit empty string literals for every `TK_STRING_LIT` token. Also added empty-pattern guard to prevent infinite loop. Runtime: 198 → 199/226
- **Self-Hosted Parser: Tuple Type → KrTupleN Mapping** (`krakenc/src/parser.kr`, `krakenc/src/platform.kr`) — Tuple type annotations `(T, U, ...)` now map to `KrTuple1`–`KrTuple8` structs instead of `void*`. Added helper functions and updated all type translation sites: `translate_var_decl`, `translate_fn`, `emit_fn_prototype`, `translate_impl_fn`, `emit_impl_prototypes`, `emit_closure_statics`
- **Self-Hosted Parser: Unsafe Block Double-`}` Fix** (`krakenc/src/parser.kr`) — `TK_KW_UNSAFE` handler no longer double-advances past `}` (once by `translate_block_body`, once manually); fixes nested-unsafe function merging. Runtime: 197 → 198/226
- **Self-Hosted Parser: Chained Closure Calls** (`krakenc/src/parser.kr`) — `level1(1)(2)(3)` now emits nested function pointer casts; pre-scans for chained `)(` sequences and wraps each call level in `((int64_t(*)(int64_t,...))...)` casts
- **Self-Hosted Parser: Closure Return Type Annotation Skip** (`krakenc/src/parser.kr`) — complex return types after `->` in closure literals (tuples, arrays, fn types) now correctly skipped during neutralization; previously only one token advanced, causing garbled C output
- **Self-Hosted Parser: Tuple Destructuring Fn Ptr Return Type** (`krakenc/src/parser.kr`) — `let (a, b) = make_ops(5)` where `make_ops` is a local fn var now emits `((KrTuple2(*)(int64_t))make_ops)(5)` with correct tuple return type instead of default `int64_t`
- **Self-Hosted Parser: `is_local_fn_var` Tuple Destructuring** (`krakenc/src/parser.kr`) — variables from `let (a, b) = ...` now recognized as local fn vars; scans inside `let (` parens for matching names
- **Self-Hosted Parser: Match Expression Arms Implicit Return** (`krakenc/src/parser.kr`) — match arms in closures now emit `return` for expression arms when match is the implicit return expression; uses `match_return` sema flag propagated from `translate_statement` to `translate_match_simple`
- **Self-Hosted Parser: Tuple Field Closure Calls** (`krakenc/src/parser.kr`) — `t.0(5)` where `t` is a tuple with closure fields now emits `((int64_t(*)(int64_t))t.f0)(5)` instead of `t.f0(5)`; detects `.INT_LIT(` pattern before identifier emission and generates full function pointer cast
- **Self-Hosted Parser: Struct Declaration Skip** (`krakenc/src/parser.kr`) — `struct Name { ... }` inside function bodies now properly skipped in `translate_statement` instead of being misinterpreted as struct literals
- **Self-Hosted Parser: Closure Detection After Colon** (`krakenc/src/parser.kr`) — closures in struct literal field values (`f: |x| x * 2`) detected via `TK_COLON` in `emit_closure_statics` prev-token check
- **Self-Hosted Parser: Struct Field Closure Call** (`krakenc/src/parser.kr`) — `c.f(21)` on closure-typed struct fields emits fat closure call via `_KR_CL_FN`/`_KR_CL_ENV` instead of method dispatch `kr_Container_f`
- **Self-Hosted Parser: Fat Closure Tuple Field Call** (`krakenc/src/parser.kr`) — `t.0(5)` on tuples with fat closures uses `_KR_CL_FN`/`_KR_CL_ENV` convention instead of direct function pointer cast; fixes bus error
- **Self-Hosted Parser: Nested Tuple Destructuring Flatten** (`krakenc/src/parser.kr`) — `let ((p, q), r) = ((10, 20), 30)` flattens both LHS and RHS using depth-tracking loop, binding leaf values to leaf names
- **Self-Hosted Parser: Variable Shadowing in Tuple Destructuring** (`krakenc/src/parser.kr`) — repeated `let (x, y) = ...` in same scope emits assignment instead of redeclaration via `tup_decl:` sema tracking
- **Self-Hosted Parser: Tuple Type Annotation Parsing** (`krakenc/src/parser.kr`) — `((int, int), (int, int))` parsed to detect sub-tuple elements and their arities for unboxing
- **Self-Hosted Parser: Tuple Element Unboxing** (`krakenc/src/parser.kr`) — destructured tuple-typed elements emit `KrTupleN name = *(KrTupleN*)(intptr_t)tmp.fN;` instead of `__auto_type` (int64_t)
- **Self-Hosted Parser: Match Nested Tuple Patterns** (`krakenc/src/parser.kr`) — `((x, y), z)` in match arms uses stack-based hierarchical field access with unboxing; supports up to 4 nesting levels
- **Self-Hosted Parser: Type-Dispatched `_KR_LEN`/`_KR_TO_STRING` Macros** (`krakenc/src/platform.kr`) — compile-time dispatch via `__builtin_choose_expr` for `.len()` (→ `strlen` or `kr_vec_int_len`) and `.to_string()` (→ identity or `kr_fmt_int`) on type-erased generic parameters
- **Self-Hosted Parser: Fallback Method Dispatch** (`krakenc/src/parser.kr`) — when receiver type is unknown (e.g., closure params from erased generics), `len()` → `_KR_LEN`, `to_string()` → `_KR_TO_STRING`, `push()` → `kr_vec_int_push` with optional tuple boxing
- **Self-Hosted Parser: Tuple Boxing in Array Push** (`krakenc/src/parser.kr`) — `result.push((a, b))` now boxes tuple literal via `_kr_box` before storing as `int64_t` in vec_int; detects tuple args by scanning for commas at depth 1
- **Self-Hosted Runtime: Null-Safe `kr_strcmp`** (`krakenc/src/platform.kr`) — guards against sub-page-size integer arguments (< 4096) by returning integer difference instead of calling `strcmp`, preventing segfaults
- **Self-Hosted Test Suite: 226/226 (100.0%)** — all programs compile and run correctly, zero failures across all categories
...
- **Compiler Diagnostic Code System** (`compiler/src/diagnostics.rs`)
  ...
  - Organized into 10 categories: Lexer (0-999), Parser (1000-1999), Type (2000-2999), Resolution (3000-3999), Borrow (4000-4999), Codegen (5000-5999), Module (6000-6999), Macro (7000-7999), Attribute (8000-8999), I/O (9000-9999)
  - Four severity levels: Error, Warning, Note, Help
  - Diagnostic metadata with descriptions and suggestions for each code
  - 80+ unique diagnostic codes covering all compiler phases
  - 11 comprehensive tests for diagnostic code functionality
- **Diagnostic Registry** (`compiler/src/diagnostic_registry.rs`)
  - Centralized registry for diagnostic code documentation and lookup
  - Detailed information for each code including title, description, examples, and fix suggestions
  - Registry validation and completeness tests (4 tests)
- **Enhanced Error System** (`compiler/src/error.rs`)
  - New DiagnosticError variant with full diagnostic context
  - Helper methods for creating diagnostic-based errors
  - Backward compatibility with legacy E-codes (E0001-E0018)
  - Seamless integration between old and new diagnostic systems

### Changed
- Compiler error system now supports both legacy error codes and new KRA diagnostic codes
- Error messages can include hints and suggestions for fixing common issues
- Module declarations updated in both lib.rs and main.rs for proper binary compilation

### Fixed
- CI/CD build failures due to missing module declarations in main.rs
- Dead code warnings in diagnostic infrastructure by adding appropriate allow attributes

### Tests
- Compiler tests: 128 passing (15 new tests for diagnostic system)
- All tests pass with zero errors and zero warnings
- Full coverage of diagnostic code creation, formatting, and registry lookup
...
- Fixed LLVM environment variable to be explicitly set per-step in build and test commands for all platforms
